...
export interface UserFormState {
  ...
  const UserSchema = z.object({
    ...
    role: z.nativeEnum(Role, { errorMap: () => ({ message: "Por favor, selecciona un rol válido." }) }),
    image: z.string().url({ message: "Por favor, introduce una URL válida para la imagen." }).or(z.literal('')).optional(), // Optional URL
  });
  ...
  export async function upsertUserAction(
    ...
    const schemaWithConditionalPassword = UserSchema.superRefine((data, ctx) => {
      if (!data.id && (!data.password || data.password.length < 6)) {
        ctx.addIssue({
          ...
          path: ['password'],
          message: 'La contraseña es requerida y debe tener al menos 6 caracteres para nuevos usuarios.',
        });
      }
      if (data.id && data.password && data.password.length > 0 && data.password.length < 6) {
         ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['password'],
          message: 'Si actualiza la contraseña, debe tener al menos 6 caracteres.',
        });
      }
    });
    ...
    try {
      if (isEdit && id) {
        ...
        if (!password) {
           return { errors: { password: ["La contraseña es requerida."] }, message: "Error", success: false, isEdit: false };
        }
